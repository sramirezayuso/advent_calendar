...
(defn turn-on-lights
  "Turn on all the lights in the given range."
  [initial-grid [a b] [v u]]
  (loop [grid initial-grid, x a]
    (cond
      (> x (count (get grid 0))) grid
      :else (recur (loop [grid grid, y b]
                     (cond
                       (> y (count grid)) grid
                       (and (<= x v) (<= y u)) (recur (assoc grid x (assoc (get grid x) y true)) (inc y))
                       :else (recur grid (inc y)))) (inc x) ))))

(defn turn-off-lights
  "Turn off all the lights in the given range."
  [initial-grid [a b] [v u]]
  (loop [grid initial-grid, x a]
    (cond
      (> x (count (get grid 0))) grid
      :else (recur (loop [grid grid, y b]
                     (cond
                       (> y (count grid)) grid
                       (and (<= x v) (<= y u)) (recur (assoc grid x (assoc (get grid x) y false)) (inc y))
                       :else (recur grid (inc y)))) (inc x) ))))

(defn toggle-lights
  "Toggle all the lights in the given range."
  [initial-grid [a b] [v u]]
  (loop [grid initial-grid, x a]
    (cond
      (> x (count (get grid 0))) grid
      :else (recur (loop [grid grid y b]
                     (cond
                       (> y (count grid)) grid
                       (and (<= x v) (<= y u)) (recur (assoc grid x (assoc (get grid x) y (not (get (get grid x) y) ))) (inc y))
                       :else (recur grid (inc y)))) (inc x) ))))

(defn numeric-turn-on-lights
  "Turn up all the lights in the given range by 1."
  [initial-grid [a b] [v u]]
  (loop [grid initial-grid, x a]
    (cond
      (> x (count (get grid 0))) grid
      :else (recur (loop [grid grid, y b]
                     (cond
                       (> y (count grid)) grid
                       (and (<= x v) (<= y u)) (recur (assoc grid x (assoc (get grid x) y (+ 1 (get (get grid x) y)))) (inc y))
                       :else (recur grid (inc y)))) (inc x) ))))

(defn numeric-turn-off-lights
  "Turn down all the lights in the given range by 1 (min 0)."
  [initial-grid [a b] [v u]]
  (loop [grid initial-grid, x a]
    (cond
      (> x (count (get grid 0))) grid
      :else (recur (loop [grid grid, y b]
                     (cond
                       (> y (count grid)) grid
                       (and (<= x v) (<= y u)) (recur (assoc grid x (assoc (get grid x) y (max (- (get (get grid x) y) 1) 0) )) (inc y))
                       :else (recur grid (inc y)))) (inc x) ))))

(defn numeric-toggle-lights
  "Turn up all the lights in the given range by 2."
  [initial-grid [a b] [v u]]
  (loop [grid initial-grid, x a]
    (cond
      (> x (count (get grid 0))) grid
      :else (recur (loop [grid grid y b]
                     (cond
                       (> y (count grid)) grid
                       (and (<= x v) (<= y u)) (recur (assoc grid x (assoc (get grid x) y (+ 2 (get (get grid x) y)))) (inc y))
                       :else (recur grid (inc y)))) (inc x) ))))

(defn create-matrix
  "Creates square matrix of the given size initialized with the given initial value."
  [size intial-value]
  (loop [i 0, matrix []]
    (if (>= i size)
      matrix
      (recur (inc i) (loop [j 0, row []]
                       (if (>= j size)
                         (conj matrix row)
                         (recur (inc j) (conj row intial-value))))))))

(defn execute-instruction
  "Execute a single light instruction."
  [grid instruction]
  (let [length (count (str/split instruction #" ")), command (get (str/split instruction #" ") (- length 4))]
    (cond
      (= command "toggle") (toggle-lights grid (map parse-int (str/split (get (str/split instruction #" ") (- length 3)) #",")) (map parse-int (str/split (get (str/split instruction #" ") (- length 1)) #",")))
      (= command "on") (turn-on-lights grid (map parse-int (str/split (get (str/split instruction #" ") (- length 3)) #",")) (map parse-int (str/split (get (str/split instruction #" ") (- length 1)) #",")))
      (= command "off") (turn-off-lights grid (map parse-int (str/split (get (str/split instruction #" ") (- length 3)) #",")) (map parse-int (str/split (get (str/split instruction #" ") (- length 1)) #","))))))

(defn execute-all-instructions
  "Execute all of the light instructions."
  [input]
  (let [commands (str/split input #"\n")]
    (loop [i 0, grid (create-matrix 1000 false)]
      (if (>= i (count commands))
        grid
        (recur (inc i) (execute-instruction grid (get commands i)))))))

(defn execute-numeric-instruction
  "Execute a numeric single light instruction."
  [grid instruction]
  (let [length (count (str/split instruction #" ")), command (get (str/split instruction #" ") (- length 4))]
    (cond
      (= command "toggle") (numeric-toggle-lights grid (map parse-int (str/split (get (str/split instruction #" ") (- length 3)) #",")) (map parse-int (str/split (get (str/split instruction #" ") (- length 1)) #",")))
      (= command "on") (numeric-turn-on-lights grid (map parse-int (str/split (get (str/split instruction #" ") (- length 3)) #",")) (map parse-int (str/split (get (str/split instruction #" ") (- length 1)) #",")))
      (= command "off") (numeric-turn-off-lights grid (map parse-int (str/split (get (str/split instruction #" ") (- length 3)) #",")) (map parse-int (str/split (get (str/split instruction #" ") (- length 1)) #","))))))

(defn execute-all-numeric-instructions
  "Execute all of the numeric light instructions."
  [input]
  (let [commands (str/split input #"\n")]
    (loop [i 0, grid (create-matrix 1000 0)]
      (if (>= i (count commands))
        grid
        (recur (inc i) (execute-numeric-instruction grid (get commands i)))))))
...
(println (count (filter identity (flatten (execute-all-instructions input)))))
(println (reduce + (flatten (execute-all-numeric-instructions input))))
